...
async def encode_video(input_file, res_key, status: Message, settings, user_id, total_duration):
    ...
    cmd = f'ffmpeg -y -i "{input_file}" {sub_cmd} -c:v {settings["codec"]} -preset {settings["preset"]} -crf {settings["crf"][res_key]} -threads 2 -max_muxing_queue_size 1024 -pix_fmt yuv420p -c:a aac -b:a {settings["audio"][res_key]} "{output_file}"'
